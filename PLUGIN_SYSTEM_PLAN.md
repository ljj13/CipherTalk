...
- 提供一个小 SDK 包（`@ciphertalk/plugin-sdk`，就是 postMessage 的类型化封装），
  插件开发者 `import { api } from '@ciphertalk/plugin-sdk'` 即可，带完整 TS 类型。
...
1. **同一套组件库**。宿主 UI 基于 `@heroui/react`（公开 npm 包）+ Tailwind，
   插件脚手架预装完全相同的版本与配置，写出来的按钮/弹窗/表单和宿主像素级一致。
2. **主题 tokens 由宿主注入**。协议提供 `ct-plugin://__runtime__/theme.css`，
   内容是宿主实时生成的当前主题变量（oklch 色板、字体、圆角、暗/亮模式）。
   SDK 自动引入该样式并监听 `themeChanged` 事件——用户切换主题，插件页同步变，
   无需插件开发者写一行适配代码。
3. **`@ciphertalk/plugin-ui` 包**（随 SDK 发布）：
   - re-export 预配置好主题绑定的 HeroUI 组件；
   - 附加宿主特有的常用组件的插件版实现（如消息气泡、联系人头像、会话选择器），
     让"做一个和宿主原生功能无差别的插件页"成为默认体验而不是高级技巧。
...
```
npm create ciphertalk-plugin   # 脚手架：Vite + React + HeroUI + SDK + 类型 + 示例视图
cd my-plugin && npm run dev    # 本地开发，HMR 热更新
npm run build                  # 产出 dist/ + manifest 校验
npm run pack                   # 打包成 my-plugin-1.0.0.ctplugin（zip）
```
...
- **安装**：`.ctplugin` 文件拖入插件管理页或文件选择安装 = 校验 manifest + 解压到
  `<userData>/plugins/`。卸载 = 删目录 + 清权限与私有存储。
- **SDK 与文档**：`@ciphertalk/plugin-sdk`（类型化 API）+ `@ciphertalk/plugin-ui`
  发布到 npm；官方文档含 quickstart、API reference、2~3 个完整示例插件
